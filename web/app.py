    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(
            ...
            if token.status == 200:
                ...
                response.set_cookie(key="token", value=JWT_token, secure=True, max_age=3600)            
                return response
...
async def process_upload(response: Response):
    ...
    content = env.get_template("upload.html").render(message="Conversion successful!", show_download=show_download)
    response = HTMLResponse(content=content)
